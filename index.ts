...
Game: while (condition) {
  ...
  console.log(
    `\t ${chalk.bold.yellow(`${enemy.name}'s Hp: ${enemy.healthPotion}`)}\n`
  );
  ...
  while (condition) {
    ...
    if (options.action === `Attack to ${enemy.name}`) {
      ...
      console.log(
        `\n${chalk.bold.magenta(
          `You strike ${enemy.name} for ${enemy.rand} damage`
        )}`
      );
      console.log(
        `${chalk.bold.magenta(
          `You recieve ${player.rand} damage in retaliaton!`
        )}`
      );
      ...
      console.log(
        chalk.red.bold(
          `${enemy.name}'s Health Portion: ${enemy.healthPotion}\n`
        )
      );
      ...
      if (
        (enemy.healthPotion <= 1 && player.healthPotion > 0) ||
    ...
    if (options.action === "Drink Health Portion") {
      ...
      console.log(
        `\t\n ${chalk.bold.magentaBright(
          `You have ${healthPotionCount} drink(s) left!`
        )}`
      );
